...
The returned `response` object follows the OpenAI schema — the embedding vector itself lives inside `response.data[...]`. Browse the object (or check the [OpenAI docs](https://developers.openai.com/api/reference/python/resources/embeddings/methods/create)) to find where exactly.
